...
async function load(){
 ...
 if(progress&&init?.mode==="two_phase_steam_year"){
  ...
  const total=Number(init.candidate_count)||0;
  ...
  const shortlisted=Number(init.priority_total??init.prefilter_matched_count)||0;
  const verified=Number(init.verified_priority_count)||0;
  ...
  const first="① Steam 官方已取得全年 "+number.format(total)+" 款";
  const second="② 第三方 ≥4,000 初篩 "+number.format(screened)+"/"+number.format(total)+" 款";
  const third=init.prefilter_complete?"③ Steam 官方驗證 "+number.format(verified)+"/"+number.format(shortlisted)+" 款":"③ Steam 官方驗證將於初篩完成後開始";
  progress.textContent=first+" · "+second+" · "+third+" · Steam 官方確認 ≥5,000："+number.format(official.count||0)+" 款 · 最後更新（台灣）"+time+"。本頁只列出未來 45 天官方驗證合格遊戲。";

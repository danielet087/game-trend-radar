...
(() => {
  ...
  function setBackLink() {
    try {
      ...
      if (
        ...
        /\/(?:index|upcoming|released|saved|date)\.html$/.test(previous.pathname)
      ) {
        $("gameBack").href = previous.pathname + previous.search + previous.hash;
      }
    } catch {
  ...
  function render(game, data) {
    ...
    $("gameCountdown").textContent = days > 0
      ? `距離預定上市還有 ${days} 天`
      : days === 0 ? "預定今天上市" : "原定上市日期已過，請以商店為準";
    $("gameState").textContent = days > 0
      ? `${days} 天後登場`
      : days === 0 ? "預定今天登場" : "上市日期已過";
  ...
  async function main() {
    ...
    if (!official && !preview) {
      setStatus("遊戲資料暫時無法讀取", "請稍後再試，或返回遊戲清單。");
      return;
    }
    try {
      ...
      setStatus("遊戲資訊暫時無法呈現", "請稍後再試，或返回遊戲清單。");
    }
  }
  main();
})();

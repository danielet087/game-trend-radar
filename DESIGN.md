...
An original discovery interface: warm paper, cobalt blue, coral accents and lime highlights. A radar motif connects the identity, cover fallback and empty states. The layout prioritizes upcoming releases, the calendar and recent releases. It does not reproduce the reference websites' page compositions.
...
New pages use only `assets/radar-data-v1.js`, `assets/radar-ui-v1.js` and `assets/radar-ui-v1.css`. Older list/date scripts remain in the repository for history but are no longer referenced by the current HTML.
